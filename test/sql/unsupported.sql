...
SELECT COALESCE(
           current_setting('pg_textsearch_test.force_unsupported_seqscan', true),
           'off'
       ) AS force_unsupported_seqscan
\gset
...
-- The test-only seqscan switch applies only here.
EXPLAIN (COSTS OFF)
SELECT id
FROM docs
...
-- =============================================================================
-- LIMITATION 4: text <@> text operator still uses the implicit index choice
-- The planner hook transforms text <@> text to text <@> bm25query, so this
-- block remains an index-scan control case alongside to_bm25query().
-- =============================================================================

\echo 'Test: text <@> text operator vs to_bm25query()'

-- text <@> text: planner transforms and still uses an index scan
EXPLAIN (COSTS OFF)
