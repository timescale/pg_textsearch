...
SELECT id, content,
       ROUND((content <@> to_bm25query('hello', 'segment_test_idx'))::numeric, 4)
       AS hello_score
FROM segment_test
WHERE content <@> to_bm25query('hello', 'segment_test_idx') < 0
ORDER BY content <@> to_bm25query('hello', 'segment_test_idx'), id;
