...
-- Note: ORDER BY must use the raw expression, not the ROUND() alias, for index scan
SELECT id, content, ROUND((content <@> to_bm25query('hello', 'implicit_docs_idx'))::numeric, 4) as score
FROM implicit_docs
ORDER BY content <@> to_bm25query('hello', 'implicit_docs_idx'), id
LIMIT 10;
...
FROM implicit_docs
ORDER BY content <@> to_bm25query('hello'), id
LIMIT 10;
...
FROM implicit_docs
ORDER BY content <@> to_bm25query($1, 'implicit_docs_idx'), id
LIMIT 10;
...
DECLARE plan json;
BEGIN
    EXECUTE 'EXPLAIN (FORMAT JSON, COSTS OFF, VERBOSE) EXECUTE prepared_score(''hello'')'
        INTO plan;
    IF plan::text NOT LIKE '%bm25_get_current_score()%' THEN
        RAISE EXCEPTION 'prepared BM25 scan did not reuse the index score: %', plan;
    END IF;
END $$;
...
DECLARE plan json;
BEGIN
    EXECUTE 'EXPLAIN (FORMAT JSON, COSTS OFF, VERBOSE) EXECUTE prepared_subquery_score(''hello'')'
        INTO plan;
    IF plan::text NOT LIKE '%bm25_get_current_score()%' THEN
        RAISE EXCEPTION 'subquery BM25 scan did not reuse the index score: %', plan;
    END IF;
END $$;
