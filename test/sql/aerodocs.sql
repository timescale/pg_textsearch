...
SELECT
    ...
    ROUND((full_text <@> to_tpvector('aerodynamic flow', 'cranfield_tapir_idx'))::numeric, 4) as score
...
SELECT
    doc_id,
    LEFT(title, 60) as title_preview,
    ROUND((full_text <@> to_tpvector('boundary layer turbulent', 'cranfield_tapir_idx'))::numeric, 4) as score
FROM aerodocs_documents
ORDER BY 3 DESC
LIMIT 5;
...
WITH tapir_results AS (
    SELECT
        doc_id,
        ROUND((full_text <@> to_tpvector((SELECT query_text FROM cranfield_queries WHERE query_id = 1), 'cranfield_tapir_idx'))::numeric, 4) as tapir_score,
        ROW_NUMBER() OVER (ORDER BY full_text <@> to_tpvector((SELECT query_text FROM cranfield_queries WHERE query_id = 1), 'cranfield_tapir_idx') DESC) as tapir_rank
    FROM aerodocs_documents
),
...
SELECT
    ...
    COUNT(CASE WHEN d.full_text <@> to_tpvector(q.query_text, 'cranfield_tapir_idx') < 0 THEN 1 END) as matching_docs
FROM cranfield_queries q
...
SET jit = off;
EXPLAIN
SELECT doc_id, ROUND((full_text <@> to_tpvector('supersonic aircraft design', 'cranfield_tapir_idx'))::numeric, 4) as score
FROM aerodocs_documents
ORDER BY 2 DESC
LIMIT 10;
