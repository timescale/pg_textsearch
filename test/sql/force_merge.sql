...
-- segment, so it can be cleared alongside an already-terminal lone L7.
DO $$
BEGIN
    IF bm25_test_chain_source(
            'force_l7_single_idx', 'one_doc_no_terms') <> 'OK' THEN
        RAISE EXCEPTION 'could not construct termless L7 memtable';
    END IF;
END
$$;
...
BEGIN
    IF regexp_count(summary, 'L[0-7] Segment [0-9]+:') <> 1
       OR summary !~ 'L7 Segment 1:'
       OR summary !~ E'Memtable:\n  terms: 0\n  documents: 1' THEN
        RAISE EXCEPTION 'termless L7 layout was not constructed: %', summary;
    END IF;
    ...
    IF regexp_count(summary, 'L[0-7] Segment [0-9]+:') <> 1
       OR summary !~ 'L7 Segment 1:'
       OR summary !~ E'Memtable:\n  terms: 0\n  documents: 0' THEN
        RAISE EXCEPTION 'termless L7 force merge changed segment layout: %',
                        summary;
    END IF;

    IF bm25_dump_index('force_l7_single_idx')::text
       !~ E'total_docs: 129\n  total_len: 519\n' THEN
        RAISE EXCEPTION 'termless L7 force merge lost document accounting';
    END IF;
    ...
    IF (SELECT count(*) FROM (
        ...
        RAISE EXCEPTION 'termless L7 force merge changed query results';
    END IF;
END
$$;
