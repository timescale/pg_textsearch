...
-- Index scan for 'alpha' must return no rows. Without the fix,
-- the stale segment entry for 'alpha' points to reused CTID (0,1)
-- and the index scan returns 'juliet kilo lima' as a false match.
SELECT id, content FROM vacuum_rebuild_test
ORDER BY content <@> to_bm25query('alpha', 'vacuum_rebuild_idx')
LIMIT 10;
