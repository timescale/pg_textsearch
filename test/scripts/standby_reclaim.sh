...
cleanup() {
    ...
    if [ -n "${COMPACTOR_BACKEND_PID}" ]; then
        primary_sql_quiet \
            "SELECT injection_points_wakeup('${POINT_AFTER_RESTAMP}');" \
            >/dev/null 2>&1 || true
    fi
    if [ -n "${SPILLER_BACKEND_PID}" ]; then
        primary_sql_quiet \
            "SELECT injection_points_wakeup(
                 '${POINT_SPILL_BEFORE_FINALIZE}');" >/dev/null 2>&1 || true
    fi
...
# Start a force merge that parks at the post-restamp injection point
# until release_compaction_pause() wakes it.
start_paused_compaction() {
    local index_name=$1
    local output=$2
    ...
    PGAPPNAME=pgts-reclaim-compactor \
        primary_sql "
            ...
            SELECT injection_points_attach('${POINT_AFTER_RESTAMP}', 'wait');
            SELECT bm25_force_merge('${index_name}');" \
        >"${output}" 2>&1 &
    COMPACTOR_PID=$!
}
...
wait_for_compaction_pause() {
    ...
    for _ in $(seq 1 300); do
        COMPACTOR_BACKEND_PID=$(primary_sql_quiet "
            SELECT pid
              FROM pg_stat_activity
             WHERE application_name = 'pgts-reclaim-compactor'
               AND wait_event_type = 'InjectionPoint'
               AND wait_event = '${POINT_AFTER_RESTAMP}'
             LIMIT 1;" 2>/dev/null || true)
        if [[ "${COMPACTOR_BACKEND_PID}" =~ ^[0-9]+$ ]]; then
...
release_compaction_pause() {
    primary_sql_quiet \
        "SELECT injection_points_wakeup('${POINT_AFTER_RESTAMP}');" \
        >/dev/null ||
        error "Could not wake ${POINT_AFTER_RESTAMP}"
    COMPACTOR_BACKEND_PID=
}
...
wait_for_spill_pause() {
    ...
    for _ in $(seq 1 300); do
        SPILLER_BACKEND_PID=$(primary_sql_quiet "
            SELECT pid
              FROM pg_stat_activity
             WHERE application_name = 'pgts-retire-horizon-spiller'
               AND wait_event_type = 'InjectionPoint'
               AND wait_event = '${POINT_SPILL_BEFORE_FINALIZE}'
             LIMIT 1;" 2>/dev/null || true)
        if [[ "${SPILLER_BACKEND_PID}" =~ ^[0-9]+$ ]]; then
...
# Killing the client is not enough: the standby backend sits in a debug
# pause and only notices the closed socket afterwards, which would stall
# the next case's shutdown.  Cancel it and wait for the backend to go.
stop_snapshot_reader() {
    local backend=$1

    kill -TERM "${SNAPSHOT_READER_PID}" 2>/dev/null || true
    if [ -n "${backend}" ]; then
        standby_sql_quiet \
            "SELECT pg_cancel_backend(${backend});" >/dev/null 2>&1 || true
        for _ in $(seq 1 600); do
            [ "$(standby_sql_quiet "
                SELECT count(*) FROM pg_stat_activity
                 WHERE pid = ${backend};" 2>/dev/null || echo 0)" = "0" ] &&
                break
            sleep 0.1
        done
    fi
...
test_atomic_segment_graph_snapshot() {
    ...
    if [ "${HAS_INJECTION_POINTS}" -ne 1 ]; then
        log "Skipping atomic segment-root snapshot case: no injection points"
        return
    fi
    ...
    start_paused_compaction snapshot_idx \
        "${PRIMARY_DIR}/snapshot_compactor.out"
    wait_for_compaction_pause "${PRIMARY_DIR}/snapshot_compactor.out"
...
test_memtable_retire_horizon_covers_prepublish_reader() {
    local dead_fxid index_oid output reader_xmin spill_output

    if [ "${HAS_INJECTION_POINTS}" -ne 1 ]; then
        log "Skipping retire-horizon case: no injection points"
        return
    fi
    ...
    rm -f "${spill_output}"
    PGAPPNAME=pgts-retire-horizon-spiller \
        psql -X -tAq -v ON_ERROR_STOP=1 -p "${PRIMARY_PORT}" \
        -d "${TEST_DB}" >"${spill_output}" 2>&1 <<SQL &
SELECT injection_points_set_local();
SELECT injection_points_attach('${POINT_SPILL_BEFORE_FINALIZE}', 'wait');
SELECT bm25_spill_index('retire_horizon_idx') > 0;
SQL
    SPILLER_PID=$!
    wait_for_spill_pause "${spill_output}"
...
SQL
    ...
    primary_sql_quiet \
        "SELECT injection_points_wakeup('${POINT_SPILL_BEFORE_FINALIZE}');" \
        >/dev/null || error "Could not wake the pre-finalize spill point"
...
reader xmin ${reader_xmin}"
    ...
    stop_snapshot_reader "${SNAPSHOT_READER_BACKEND_PID}"
...
test_late_ranked_cursor_reclaim() {
    local plan first_id remaining
    ...
    if [ "${HAS_INJECTION_POINTS}" -ne 1 ]; then
        log "Skipping late ranked cursor reclaim: no injection points"
        return
    fi

    start_paused_compaction rec_idx "${PRIMARY_DIR}/compactor.out"
    wait_for_compaction_pause "${PRIMARY_DIR}/compactor.out"
...
main() {
    local spilled graph feedback_setting
