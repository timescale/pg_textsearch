...
typedef struct TpStringKey
{
	...
	dsa_pointer flag_field;	   /* posting_list_dp for table entries, 0 for
								* lookup keys */
} TpStringKey;
...
struct TpStringHashEntry
{
	...
	int32 posting_list_len; /* Number of documents containing this
							 * term */
...
extern TpStringHashEntry *tp_hash_lookup_dsa(
		dsa_area *area, TpStringHashTable *ht, const char *str, size_t len);
extern TpStringHashEntry *tp_hash_insert_dsa(
		dsa_area *area, TpStringHashTable *ht, const char *str, size_t len);
extern bool tp_hash_delete_dsa(
		dsa_area *area, TpStringHashTable *ht, const char *str, size_t len);
