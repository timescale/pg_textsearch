...
/*
 * tp_check_segment_consistency - debug-only segment skip-data consistency
 * check
 *
 * TEMPORARY DIAGNOSTIC: surfaces inconsistencies between cached skip data
 * (skip_entry.last_doc_id, dict_entry.block_count) and the actual on-disk
 * block_postings. Built to investigate the MS MARCO bucket-8 hang
 * (PR #360 follow-up): seek_term_to_doc enters a no-progress path when
 * skip-data caches lie about a block's last doc id.
 *
 * For each segment in each level, for each term in the dictionary,
 * iterates every block via the BMW posting iterator (handles compression)
 * and verifies:
 *
 *   1. iter.skip_entry.last_doc_id == block_postings[doc_count-1].doc_id
 *   2. block_postings is sorted strictly ascending by doc_id
 *   3. first doc of block N > last doc of block N-1 (monotonic across blocks)
 *
 * Returns a text report listing the first few inconsistencies per segment
 * plus aggregate counts. Superuser-only.
 *
 * This is *not* meant to be a permanent feature; once we find/fix the
 * underlying skip-data corruption in segment build/merge, this function
 * should be removed.
 */
PG_FUNCTION_INFO_V1(tp_check_segment_consistency);

Datum
tp_check_segment_consistency(PG_FUNCTION_ARGS)
{
	text		   *index_name_text = PG_GETARG_TEXT_PP(0);
	char		   *index_name		= text_to_cstring(index_name_text);
	Oid				index_oid;
	...
	const int		max_examples_per_seg  = 8;

	if (!superuser())
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("must be superuser to check segment consistency")));
	...
	appendStringInfo(
			&result, "Segment consistency check for '%s'\n", index_name);
	...
	for (level = 0; level < TP_MAX_LEVELS; level++)
	{
		...
		while (seg_root != InvalidBlockNumber)
		{
			...
			for (t = 0; t < dict_header.num_terms; t++)
			{
				char					*term_text;
				TpDictEntry				 entry;
				...
				term_text = tp_segment_read_term_at_index(
						reader, reader->header, string_offsets, t);
				...
				tp_segment_read_dict_entry(reader, reader->header, t, &entry);
				...
				if (!tp_segment_posting_iterator_init(
							&iter, reader, term_text))
				{
					pfree(term_text);
					continue;
				}

				for (block_idx = 0; block_idx < entry.block_count; block_idx++)
				{
					uint32 actual_last;
					uint32 k;
					bool   sorted_ok = true;
					...
					if (iter.skip_entry.doc_count == 0)
						continue;

					/* Check 1: skip.last_doc_id vs actual last posting */
					actual_last =
							iter.block_postings[iter.skip_entry.doc_count - 1]
									.doc_id;
					if (actual_last != iter.skip_entry.last_doc_id)
					{
						total_inconsistencies++;
						seg_inconsistencies++;
						if (seg_examples < max_examples_per_seg)
						{
							appendStringInfo(
									&result,
									"  L%d seg=%u term='%s' block=%u: "
									"skip.last_doc_id=%u vs "
									"postings[%u].doc_id=%u\n",
									level,
									seg_root,
									term_text,
									...
									iter.skip_entry.doc_count - 1,
									actual_last);
							seg_examples++;
						}
					}

					/* Check 2: intra-block sorted ascending */
					for (k = 1; k < iter.skip_entry.doc_count; k++)
					{
						if (iter.block_postings[k].doc_id <=
							iter.block_postings[k - 1].doc_id)
						{
							sorted_ok = false;
							total_inconsistencies++;
							seg_inconsistencies++;
							if (seg_examples < max_examples_per_seg)
							{
								appendStringInfo(
										&result,
										"  L%d seg=%u term='%s' block=%u: "
										"not strictly ascending at pos %u "
										"(prev=%u, cur=%u)\n",
										level,
										seg_root,
										term_text,
					...
					/* Check 3: monotonic across blocks */
					if (!first_block &&
						iter.block_postings[0].doc_id <= prev_last_doc)
					{
						total_inconsistencies++;
						seg_inconsistencies++;
						if (seg_examples < max_examples_per_seg)
					...
					first_block	  = false;
					prev_last_doc = iter.skip_entry.last_doc_id;
				}
			...
			if (seg_inconsistencies > seg_examples)
				appendStringInfo(
						&result,
						"  L%d seg=%u: ... %d more inconsistencies "
						"(showing first %d)\n",
						level,
						seg_root,
						seg_inconsistencies - seg_examples,
						seg_examples);
	...
	appendStringInfo(
			&result,
			"\nChecked %d segments, %d terms, %d blocks. "
