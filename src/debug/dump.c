...
PG_FUNCTION_INFO_V1(tp_test_hold_index_lock);

/*
 * Hold the per-index lock for a while, returning the count of
 * exclusive admissions observed once this caller holds it.  A shared
 * caller that queued behind an exclusive waiter must observe the
 * count advance across its wait.
 */
Datum
tp_test_hold_index_lock(PG_FUNCTION_ARGS)
{
	text			  *index_name_text = PG_GETARG_TEXT_PP(0);
	char			  *index_name	   = text_to_cstring(index_name_text);
	bool			   exclusive	   = PG_GETARG_BOOL(1);
	...
	if (!superuser())
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("must be superuser to hold an index test lock")));
	if (milliseconds < 0 || milliseconds > 60000)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("lock hold duration must be between 0 and 60000 ms")));
	...
	PG_TRY();
	{
		tp_acquire_index_lock(
				index_state, exclusive ? LW_EXCLUSIVE : LW_SHARED);
		admissions = pg_atomic_read_u64(
				&index_state->shared->exclusive_admissions);
		if (milliseconds > 0)
	...
	PG_RETURN_INT64((int64)admissions);
}

PG_FUNCTION_INFO_V1(tp_test_exclusive_waiters);

Datum
tp_test_exclusive_waiters(PG_FUNCTION_ARGS)
{
	text			  *index_name_text = PG_GETARG_TEXT_PP(0);
	char			  *index_name	   = text_to_cstring(index_name_text);
	Oid				   index_oid;
	...
	if (!superuser())
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("must be superuser to inspect index test waiters")));
	...
	waiters = pg_atomic_read_u32(&index_state->shared->exclusive_waiters);
	index_close(index_rel, AccessShareLock);

	PG_RETURN_INT64((int64)waiters);
}

PG_FUNCTION_INFO_V1(tp_test_exclusive_admissions);

/*
 * Read the per-index exclusive admission count without taking the
 * lock, so a caller can sample it while a writer holds it.
 */
Datum
tp_test_exclusive_admissions(PG_FUNCTION_ARGS)
{
	text			  *index_name_text = PG_GETARG_TEXT_PP(0);
	char			  *index_name	   = text_to_cstring(index_name_text);
	Oid				   index_oid;
	...
	if (!superuser())
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("must be superuser to inspect index test "
						"admissions")));
