...
#define TP_DEFAULT_INDEX_MEMORY_LIMIT \
	64 /* Default 64MB per index (not \
		* enforced) */
#define TP_MAX_INDEX_MEMORY_LIMIT             \
	512 /* Max 512MB per index (not enforced) \
		 */
...
#define TP_DEFAULT_INDEX_SELECTIVITY       \
	0.1 /* Assume 10% selectivity for text \
		 * searches */
...
#define TAPIR_TRANCHE_DOC_LENGTHS      \
	1004 /* Document length hash table \
		  * operations */
