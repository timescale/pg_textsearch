...
tp_fsm_claim_or_extend_block(Relation index)
{
	...
	/*
	 * FSM has no reusable page: extend through the same bulk-extension API
	 * as the memtable allocator.  Mixing ReadBufferExtended(P_NEW) with
	 * ExtendBufferedRel lets concurrent compaction and memtable growth
	 * reserve the same block on PostgreSQL 17.  EB_LOCK_FIRST returns the
	 * zero-filled page pinned and exclusively locked.
	 */
