...
INSERT INTO msmarco_qrels_paradedb (query_id, passage_id, relevance)
...
SELECT 'Passages loaded:' as metric, COUNT(*)::text as count FROM msmarco_passages_paradedb
UNION ALL
SELECT 'Queries loaded:', COUNT(*)::text FROM msmarco_queries_paradedb
UNION ALL
SELECT 'Relevance judgments:', COUNT(*)::text FROM msmarco_qrels_paradedb
ORDER BY metric;
...
SELECT
    ...
    pg_relation_size('msmarco_paradedb_idx') as index_bytes;
SELECT
    'TABLE_SIZE:' as label,
    pg_size_pretty(pg_total_relation_size('msmarco_passages_paradedb')) as table_size,
