...
DROP TABLE IF EXISTS benchmark_queries_systemy;
CREATE TABLE benchmark_queries_systemy (
    query_id INTEGER,
    query_text TEXT,
    token_bucket INTEGER
);
\copy benchmark_queries_systemy FROM 'benchmarks/datasets/msmarco/benchmark_queries.tsv' WITH (FORMAT text, DELIMITER E'\t')

-- Verify load
SELECT 'Loaded ' || COUNT(*) || ' benchmark queries' as status FROM benchmark_queries_systemy;
SELECT token_bucket, COUNT(*) as count FROM benchmark_queries_systemy GROUP BY token_bucket ORDER BY token_bucket;
...
DECLARE
    q record;
BEGIN
    FOR q IN SELECT query_text FROM benchmark_queries_systemy ORDER BY random() LIMIT 50 LOOP
        EXECUTE 'SELECT passage_id FROM msmarco_passages_systemy
                 WHERE embedding IS NOT NULL
                 ORDER BY embedding <&> to_bm25query(''msmarco_systemy_idx'', tokenize($1, ''msmarco_tokenizer''))
...
BEGIN
    ...
    FOR q IN SELECT query_text FROM benchmark_queries_systemy WHERE token_bucket = bucket ORDER BY query_id LOOP
        start_ts := clock_timestamp();
        EXECUTE 'SELECT COUNT(*) FROM (SELECT passage_id FROM msmarco_passages_systemy
                 WHERE embedding IS NOT NULL
                 ORDER BY embedding <&> to_bm25query(''msmarco_systemy_idx'', tokenize($1, ''msmarco_tokenizer''))
...
FROM benchmark_bucket_systemy(1);
...
FROM benchmark_bucket_systemy(2);
...
FROM benchmark_bucket_systemy(3);
...
FROM benchmark_bucket_systemy(4);
...
FROM benchmark_bucket_systemy(5);
...
FROM benchmark_bucket_systemy(6);
...
FROM benchmark_bucket_systemy(7);
...
FROM benchmark_bucket_systemy(8);

DROP FUNCTION benchmark_bucket_systemy;
...
DECLARE
    ...
    query_count int;
BEGIN
    SELECT COUNT(*) INTO query_count FROM benchmark_queries_systemy;

    -- Warmup: run all queries once
    FOR q IN SELECT query_text FROM benchmark_queries_systemy ORDER BY query_id LOOP
        EXECUTE 'SELECT passage_id FROM msmarco_passages_systemy
                 WHERE embedding IS NOT NULL
                 ORDER BY embedding <&> to_bm25query(''msmarco_systemy_idx'', tokenize($1, ''msmarco_tokenizer''))
                 LIMIT 10' USING q.query_text;
...
FROM benchmark_throughput_systemy();

DROP FUNCTION benchmark_throughput_systemy;
...
SELECT
    'msmarco_systemy_idx' as index_name,
    pg_size_pretty(pg_relation_size('msmarco_systemy_idx')) as index_size,
    pg_size_pretty(pg_relation_size('msmarco_passages_systemy')) as table_size,
    (SELECT COUNT(*) FROM msmarco_passages_systemy) as num_documents;
