-- Wikipedia Dataset - Load and Index (ParadeDB)
-- Loads Wikipedia articles and creates ParadeDB BM25 index
--
-- Usage:
...
\echo '=== Wikipedia Dataset - Data Loading (ParadeDB) ==='
\echo ''
...
-- Create ParadeDB BM25 index with English tokenizer
\echo ''
\echo '=== Building ParadeDB BM25 Index ==='
\echo 'Creating ParadeDB BM25 index on Wikipedia articles...'
CREATE INDEX wikipedia_paradedb_idx ON wikipedia_articles_paradedb
...
SELECT
    'INDEX_SIZE:' as label,
    pg_size_pretty(pg_relation_size('wikipedia_paradedb_idx')) as index_size,
    pg_relation_size('wikipedia_paradedb_idx') as index_bytes;
SELECT
    'TABLE_SIZE:' as label,
    pg_size_pretty(pg_total_relation_size('wikipedia_articles_paradedb')) as table_size,
    pg_total_relation_size('wikipedia_articles_paradedb') as table_bytes;

\echo ''
\echo '=== Wikipedia Load Complete (ParadeDB) ==='
