...
--   precompute_ground_truth.sql, from ground_truth_pgNN.tsv files)
...
--   - passage_id is TEXT (not INTEGER)
--   - Uses msmarco_v2_passages / msmarco_v2_bm25_idx
--   - ground_truth.doc_id is TEXT

\set ON_ERROR_STOP on
\timing on

\echo '=== MS-MARCO v2 BM25 Validation ==='
\echo ''
...
\copy ground_truth FROM 'benchmarks/datasets/msmarco-v2/ground_truth.tsv' WITH (FORMAT text, DELIMITER E'\t', HEADER true)

SELECT 'Loaded ' || COUNT(DISTINCT query_id) || ' queries, '
    || COUNT(*) || ' result rows' as status
FROM ground_truth;

-- Validation function that handles ties properly.
-- Uses absolute tolerance (default 0.001) to account for float4 vs
-- float8 precision differences between index scoring and ground
-- truth. Rank-boundary ties (missing/extra docs with scores within
-- tolerance of the rank-10 cutoff) are counted separately from real
-- mismatches.
CREATE OR REPLACE FUNCTION validate_single_query_v2(
    p_query_id int,
    p_query_text text,
...
BEGIN
    ...
    SELECT
        ...
        -(passage_text <@> to_bm25query(
            p_query_text, 'msmarco_v2_bm25_idx'
        ))::float8 as score
    ...
    -- Get document sets
    SELECT array_agg(gt.doc_id ORDER BY gt.doc_id)
    INTO v_gt_docs
    ...
    WHERE d NOT IN (
        SELECT gt2.doc_id FROM ground_truth gt2
        WHERE gt2.query_id = p_query_id
    );

    -- Compare scores for matching documents (absolute tolerance)
    FOR r IN
        SELECT
            gt.doc_id,
    ...
    LOOP
        IF r.abs_diff > p_tolerance THEN
            ...
            v_details := v_details || 'doc ' || r.doc_id ||
                ': gt=' || ROUND(r.gt_score::numeric, 4)::text ||
                ', tapir=' ||
                ROUND(r.tapir_score::numeric, 4)::text || '; ';
        END IF;
        v_max_score_diff := GREATEST(
            v_max_score_diff, r.abs_diff
        );
    END LOOP;

    -- Check if missing/extra docs are rank-boundary ties.
    -- If the missing doc's gt score is within tolerance of the
    -- rank-10 boundary score, it's a benign tie-break difference.
    IF v_missing IS NOT NULL
        AND array_length(v_missing, 1) > 0
    THEN
        SELECT MIN(gt3.score) INTO v_gt_min_score
        FROM ground_truth gt3
        WHERE gt3.query_id = p_query_id;
        SELECT MIN(score) INTO v_tapir_min_score
        FROM tapir_results;
        v_boundary_tolerance := GREATEST(
            p_tolerance, 0.001
        );
        ...
        LOOP
            -- Missing doc's score close to boundary = tie-break
            IF ABS(r.gt_score - v_tapir_min_score)
                > v_boundary_tolerance
            THEN
    ...
    END IF;
    IF v_extra IS NOT NULL
        AND array_length(v_extra, 1) > 0
    THEN
...
SELECT v.*
FROM (
    SELECT DISTINCT gt.query_id, gt.query_text
    FROM ground_truth gt
) q,
LATERAL validate_single_query_v2(
...
SELECT
    ...
    SUM(CASE WHEN docs_match THEN 1 ELSE 0 END)
        as docs_match_count,
    SUM(CASE WHEN scores_match THEN 1 ELSE 0 END)
        as scores_match_count,
    round(100.0 * SUM(
        CASE WHEN docs_match THEN 1 ELSE 0 END
    ) / NULLIF(COUNT(*), 0), 1) as docs_match_pct,
    round(100.0 * SUM(
        CASE WHEN scores_match THEN 1 ELSE 0 END
    ) / NULLIF(COUNT(*), 0), 1) as scores_match_pct,
...
BEGIN
    ...
    -- A query fails if scores differ beyond tolerance
    -- OR if docs differ for non-tie-break reasons
    SELECT COUNT(*) INTO v_failures
    FROM validation_results
    WHERE NOT scores_match OR NOT docs_match;

    v_match_pct := 100.0 * (v_total - v_failures)
        / NULLIF(v_total, 0);

    IF v_match_pct < 100.0 THEN
        RAISE NOTICE 'VALIDATION FAILED: % of % queries failed '
            '(scores differ beyond 0.001 tolerance or '
            'non-tie doc mismatches)',
            v_failures, v_total;
    ELSE
        RAISE NOTICE 'VALIDATION PASSED: All % queries match '
            'within tolerance', v_total;
    END IF;
END;
$$;
...
DROP FUNCTION IF EXISTS validate_single_query_v2;
