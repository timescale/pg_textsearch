...
# queries from MS-MARCO v2 Dev1, bucketed by the number of search tokens
# after stop word removal and stemming (via to_tsvector).
#
...
INPUT_FILE="$DATA_DIR/passv2_dev_queries.tsv"
TEMP_FILE=$(mktemp)
...
if [ ! -f "$INPUT_FILE" ]; then
    ...
    echo "Run ./download.sh first to get the MS-MARCO v2 dataset"
    exit 1
fi
...
psql -d postgres -p 5432 -t -A -F $'	' -q -v ON_ERROR_STOP=1 << EOSQL | grep -v "^CREATE\|^COPY" > "$OUTPUT_FILE"
