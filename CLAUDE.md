...
| `pg_textsearch.compaction_mode` | Spill-time compaction: `inline`, `background` (dispatch a callback at pre-commit), or `off` | inline |
| `pg_textsearch.compaction_request_function` | Function taking one `regclass`, invoked in `background` mode | (empty) |
