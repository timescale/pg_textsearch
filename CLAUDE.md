...
| `pg_textsearch.enable_facet_pushdown` | Push a selective `col OP const` filter into BMW so the top-k reflects only matching rows | true |
| `pg_textsearch.facet_selectivity_threshold` | Max estimated selectivity for facet pushdown (pushdown/baseline crossover; run `ANALYZE`) | 0.02 |
| `pg_textsearch.log_facet` | Log whether the facet allow-list was built via an index scan on the facet column or a heap scan | false |
